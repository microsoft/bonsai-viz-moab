...
export class MoabRenderer extends Component<
  MoabRendererProps,
  MoabRendererState
> {
  ...
  componentDidMount(): void {
    ...
    this._calibrationMarker = calibrationMarkerArcs.map((arc, index) => {
      ...
      const markerPos = new THREE.Vector3(
        0.012 * Math.cos(moveAngle),
        0.012 * Math.sin(moveAngle),
        0.004
      );
        ...
        gltfLoader.load("./moabDevice.glb", (object: GLTF) => {
          object.scene.traverse((child: THREE.Object3D) => {
            ...
            if (child.name === "ARM_ASSEMBLY") {
              ...
              this._armLeft.applyMatrix4(
                new THREE.Matrix4().makeRotationAxis(zAxis, angle)
              );
            ...
            if (child.name === "_BALANCE_PLATE^BALANCE_PLATE_ASSEMBLY001") {
              const plateMat = (child as THREE.Mesh)
  ...
  private _anglesForArmJoints(v: number, h: number) {
    ...
    const theta_link = Math.acos(
      (Math.pow(a, 2) + Math.pow(b, 2) - Math.pow(c, 2)) / (2 * a * b)
    );
    const theta_ac = Math.acos(
    ...
    const theta_servo =
      h >= 0 ? theta_hc - theta_ac : Math.PI - (theta_hc + theta_ac);
  ...
  private _orientArm(
    armGroup: THREE.Object3D,
    platePos: THREE.Vector3,
    plateQ: THREE.Quaternion
  ) {
  ...
  private _render3DScene() {
    ...
    const plateNor = new THREE.Vector3(
      moabState.plateNorX,
      moabState.plateNorY,
      moabState.plateNorZ
    );
    ...
    const ballPos = new THREE.Vector3(
      ...
      moabState.ballPosY,
      moabState.ballPosZ
    ).add(this._plateSurfaceOrig);

    // target
    const targetPos = new THREE.Vector3(
      moabState.targetPosX,
      moabState.targetPosY,
      PlateOriginToSurfaceOffset
    );

    // obstacle
    const obstaclePos = new THREE.Vector3(
      moabState.obstaclePosX,
      moabState.obstaclePosY,
      0.0
    );
    ...
    if (this._estimatedVel) {
      if (
        moabState.estimatedVelX !== undefined &&
        moabState.estimatedVelY !== undefined
      ) {
        const estimatedVel = new THREE.Vector3(
          moabState.estimatedVelX,
          moabState.estimatedVelY,
          0
        );
